...
function initSquares() {
  ...
  for (let y = 0; y < 8; y++) {
    let squarePosition = {
      x: y * props.boardSettings.square.width + props.boardSettings.padding,
      y:
        0 * props.boardSettings.square.height +
        props.boardSettings.padding -
        20,
    };
    let code = "black-" + y;
    ...
    squarePosition = {
      x: y * props.boardSettings.square.width + props.boardSettings.padding,
      y:
        9 * props.boardSettings.square.height +
        props.boardSettings.padding +
        60,
    };
    code = "white-" + y;
...
function onMouseMove(e) {
  ...
  if (
    color.value.light != store.state.whiteSquares ||
    color.value.dark != store.state.blackSquares
  ) {
    // setTimeout(() => {
    for (let i = 0; i < 8; i++) {
      for (let j = 0; j < 8; j++) {
...
// ionViewDidEnter: () => {
//       // if (Game.color.value.light != store.state.whiteSquares || Game.color.value.dark != store.state.blackSquares) {
//       //   for(let i = 0; i < 8; i++){
//       //     for(let j = 0; j < 8; j++){
//       //       ((i + j) % 2)
//       //       ? Game.color.value.dark = store.state.blackSquares
//       //       : Game.color.value.light = store.state.whiteSquares;
//       //     }
//       //   }
//       // }
...
function printBoard() {
  ...
  let szachownica = reactive([]);
  for (let i = 0; i < 8; i++) {
    szachownica.push([]);
      for (let j = 0; j < 8; j++) {
        squares[i][j].content.piece
        ? szachownica[i].push(squares[i][j].content.piece)
        : szachownica[i].push(" ");
      }
  }
